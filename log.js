$(document).ready(function () {
    const APIKEY = "63ec8821478852088da682a2";
     ...
     $("#contact-submit").on("click", function (e) {
       ...
       let contactEmail = $("#account-email").val();
       let contactPassword = $("#password").val();
       ...
       $.ajax(settings).done(function (response) {
         ...
         loginButton.addEventListener('click', event => {
           ...
           for (var i = 0; i < response.length && i < limit; i++) {
            if (emailLogin == response[i].email ) {
               if ( passwordLogin === response[i].password){
               ...
               //alert('log in is not successfull');
         ...
         for (var i = 0; i < response.length && i < limit; i++) {
           ...
           content = `${content}<tr id='${response[i]._id}'><td>${response[i].email}</td>
